...
> **policy の版について。** この検証は v22 で通してある。学習側はその後
> v23（v22 の設定シャドーイング修正）と v24（観測ロバスト化）へ進んでいるが、
> **sim2sim の目的はブリッジの検証で policy の版は問わない**ので、
> ここの段 1-5 の結果はそのまま有効。段 6 も v22 で通してよい。
> 実機に載せる版が決まったら `make export` で差し替えて段 1-3 を再実行する。
