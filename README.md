...
> **取り込み履歴は [docs/version_history_ja.md](docs/version_history_ja.md)。**
> どの policy 版がいつ入って各段が何点だったかはそちらにまとめてある。
> 新しい版を `make export` したらあの表に 1 行足す。
>
> `isaac_policy_weights.h` の先頭に**来歴ブロック**が焼き込まれている
> （版・親の版・変更点・5 条件の実測・留意点）。実機に載っている重みが
> どういうものかはヘッダを見れば分かる。出所は学習側 params の `provenance:` ブロック。
...
### 段 6 の突き合わせ先（Isaac 側 v24 の実測、320 エピソード）

**この表と比べるのが段 6。5 条件とも Isaac 側は転倒 0。**

| 指標 | Isaac (v24) | big_rabbit_mujoco |
|---|---:|---|
| 前進速度 | 0.210 m/s | 未計測 |
| 後退速度 | -0.161 | 未計測 |
| 左旋回 | 0.580 rad/s | 未計測 |
...
| 静止時 接地率 | 0.995 | 未計測 |
| 静止時 速度 | 0.004 | 未計測 |
| 足上げピーク（前進）| 0.077 m | 未計測 |
| 骨盤高（前進 平均）| 0.432 m | 未計測 |
| 滑り速度（接地中）| 0.046 m/s | 未計測 |
| トルク使用率 平均/最大 | 0.182 / 0.596 | 未計測 |

版ごとの推移は [docs/version_history_ja.md](docs/version_history_ja.md) の表を見る。
